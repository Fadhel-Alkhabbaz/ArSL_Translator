...
def describe_image_for_blind(image_bytes, api_key):
    """يرسل صورة مرفوعة من المستخدم لنموذج رؤية-لغة (VLM) ويطلب وصفًا عامًا
    وموجزًا مناسبًا لشخص كفيف — يركّز على أهم ما في الصورة فقط، وليس كل
    تفصيل دقيق فيها (حتى لا يطول الاستماع بلا داعٍ).
    """
    import base64
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "البصر على فهم محتوى الصور بسرعة. أعطِ وصفًا عامًا وموجزًا "
                    "(3-4 جمل كحد أقصى) يركّز فقط على أهم ما في الصورة: الموضوع "
                    "الرئيسي، أهم الأشخاص أو العناصر، والسياق العام (أين/ماذا "
                    "يحدث). تجاهل التفاصيل الدقيقة غير المهمة (مثل الألوان "
                    "الدقيقة لكل عنصر صغير أو الخلفية الثانوية)، إلا إذا كانت "
                    "جوهرية لفهم الصورة. إذا وُجد نص مهم وواضح داخل الصورة "
                    "(كلافتة أو عنوان)، اذكره باختصار. استخدم جملًا مباشرة "
                    "وواضحة بدون مقدمات."
                ),
            },
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "صف هذه الصورة بشكل عام، وركّز فقط على الأهم فيها."},
...
/* نطبّق الاتجاه العربي (RTL) فقط على محتوى الصفحة الرئيسي، وليس على هيكل
   الواجهة نفسه (الشريط الجانبي وآلية طيّه) — حتى لا ينكسر شكله عند الإغلاق */
